(function () {
    ...
    angular
        .module('app')
        .controller('RegistrationOnlineResultController', RegistrationOnlineResultController);

    RegistrationOnlineResultController.$inject = ['$scope', 'adminService', 'Factory', 'ConstantData', '$location'];

    function RegistrationOnlineResultController($scope, adminService, Factory, ConstantData, $location) {
        ...
        $scope.getResultTypeList = function () {
            $scope.dataLoading = true;
            adminService.getResultTypeList(1, $scope.ClassNo)
                .then(function (response) {
                    if (response.data.Message == 'Success') {
        ...
        $scope.getRegistrationOnlineList = function () {
            ...
            console.log(obj);
            $scope.dataLoading = true;
            adminService.getRegistrationOnlineListForResult(obj)
                .then(function (response) {
                    if (response.data.Message == 'Success') {
        ...
        $scope.saveRegistrationOnlineResult = function () {
            $scope.dataLoading = true;
            adminService.saveRegistrationOnlineResult($scope.RegistrationOnlineList)
                .then(function (response) {
                    if (response.data.Message == 'Success') {
