(function () {
    ...
    angular
        .module('app')
        .controller('TransferCertificateListController', TransferCertificateListController);

    TransferCertificateListController.$inject = ['$scope', 'adminService', 'Factory', 'ConstantData'];

    function TransferCertificateListController($scope, adminService, Factory, ConstantData) {
        ...
        $scope.filterList = function () {
            $scope.TransferCertificateList = [];
            angular.forEach($scope.MainTransferCertificateList, function (admission, key) {
                if ($scope.ClassId == 0) {
                    $scope.TransferCertificateList.push(admission);
        ...
        $scope.getTransferCertificateList = function () {
            $scope.dataLoading = true;
            adminService.getTransferCertificateList($scope.AcademicSessionId,0,0)
                .then(function (response) {
                    if (response.data.Message == 'Success') {
                        $scope.MainTransferCertificateList = response.data.TransferCertificateList;
                        $scope.filterList();
                    } else {
                        toastr.error(response.data.Message);
                    }
