(function () {
    ...
    angular
        .module('app')
        .controller('AdmitCardListForORController', AdmitCardListForORController);

    AdmitCardListForORController.$inject = ['$scope', 'adminService', 'Factory', 'ConstantData', '$location'];

    function AdmitCardListForORController($scope, adminService, Factory, ConstantData, $location) {
        ...
        $scope.sendSMS = function (obj) {
            ...
            adminService.sendAdmitCardSMSRO(obj)
        ...
        $scope.sendAdmitCardSMSRO = function () {
            var obj = {
                ...
                IsSameSchool: ($scope.IsSameSchool == 1 ? true : ($scope.IsSameSchool == 0 ? false : null)),
            }
            $scope.dataLoading = true;
            adminService.sendAdmitCardSMSRO(obj)
                .then(function (response) {
        ...
        $scope.getSubjectOptionList = function () {
            if ($scope.AllocatedClassStreamTypeId == null || $scope.AllocatedClassStreamTypeId == undefined) {
                return;
            }

            $scope.dataLoading = true;
            adminService.getSubjectOptionList($scope.ClassNo, $scope.AllocatedClassStreamTypeId, 0)
                .then(function (response) {
                    if (response.data.Message == 'Success') {
        ...
        $scope.getRegistrationOnlineList = function () {
            ...
            $scope.dataLoading = true;
            adminService.getRegistrationOnlineListForAdmitCard(obj)
                .then(function (response) {
                    if (response.data.Message == 'Success') {
