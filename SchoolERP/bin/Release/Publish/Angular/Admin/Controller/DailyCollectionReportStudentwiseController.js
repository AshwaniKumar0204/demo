(function () {
    ...
    angular
        .module('app')
        .controller('DailyCollectionReportStudentwiseController', DailyCollectionReportStudentwiseController);

    DailyCollectionReportStudentwiseController.$inject = ['$scope', 'adminService', 'Factory', 'ConstantData', 'loadDataService'];

    function DailyCollectionReportStudentwiseController($scope, adminService, Factory, ConstantData, loadDataService) {
        ...
        $scope.getDailyCollectionReport = function () {
            ...
            $scope.dataLoading = true;
            adminService.getDailyCollectionReportStudentwise(DCRRequest)
                .then(function (response) {
                    if (response.data.Message == 'Success') {
                        $scope.DailyCollectionReport = response.data.DailyCollectionReport;
                        $scope.DailyCollectionReport.MainTableItemList = response.data.DailyCollectionReport.TableItemList;
                        $scope.filterList();
        ...
        $scope.filterList = function () {
            $scope.DailyCollectionReport.TableItemList = [];
            if ($scope.SessionName != null && $scope.SessionName != undefined) {
                angular.forEach($scope.DailyCollectionReport.MainTableItemList, function (d1, k1) {
                    if (d1.DynamicList[5].Value == $scope.SessionName) {
                        $scope.DailyCollectionReport.TableItemList.push(d1)
                ...
                $scope.DailyCollectionReport.TableItemList = $scope.DailyCollectionReport.MainTableItemList;
            }
        }

        $scope.getDailyCollectionReport2 = function () {
            $scope.IsSubmitted = true;
            if (!$scope.IsFormDCRValid) {
                toastr.error("From Date and To Date is required!!");
                return;
            ...
            $scope.dataLoading = true;
            adminService.getDailyCollectionReportStudentwise2(DCRRequest)
                .then(function (response) {
                    if (response.data.Message == 'Success') {
                        $scope.DailyCollectionReport = response.data.DailyCollectionReport;
                        if ($scope.DailyCollectionReport.TableItemList.length == 0) {
