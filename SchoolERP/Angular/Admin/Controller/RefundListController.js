(function () {
    ...
    RefundListController.$inject = ['$scope', 'adminService', 'Factory', 'ConstantData', 'loadDataService'];

    function RefundListController($scope, adminService, Factory, ConstantData, loadDataService) {
        ...
        $scope.getRegistrationOnlineList = function () {
            ...
            var obj = {
                ...
                RefundStatus: $scope.RefundStatus,
            }
            $scope.dataLoading = true;
        ...
        $scope.updateRefundStatus = function () {
            var RegistrationList = [];
            angular.forEach($scope.RegistrationOnlineList, function (registration, key1) {
                RegistrationList.push({ RefundId: registration.RefundId, RefundStatus: registration.RefundStatus });
            })

            $scope.dataLoading = true;
            adminService.updateRefundStatus(RegistrationList)
                .then(function (response) {
                    if (response.data.Message == 'Success') {
